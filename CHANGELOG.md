...
- The font-switching commands `\mathbf`, `\mathbb`, `\mathsf`, `\mathtt`, `\mathfrak`, `\mathscr`, `\textrm`, `\textbf`, `\textit`, `\textsf`, `\texttt`, and `\textsc`. The bundled set has no bold, sans-serif, monospace or Fraktur face, so those four map to the nearest face that does exist and render as upright roman (`\textsc` does really set small letters as capitals). `\mathbb` and `\mathscr` are not among them - both are real alphabets now, see the `msbm10` and `rsfs10` entries below. The `\text*` family treats its argument as text, so the spaces in it survive, as `\text` already did.
