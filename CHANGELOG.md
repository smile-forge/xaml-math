...
- The font-switching commands `\mathbf`, `\mathbb`, `\mathsf`, `\mathtt`, `\mathfrak`, `\mathscr`, `\textrm`, `\textbf`, `\textit`, `\textsf`, `\texttt`, and `\textsc`. The bundled set has no bold, sans-serif, monospace or Fraktur face, so those map to the nearest face that does exist and render as upright roman (`\mathscr` is genuinely the calligraphic alphabet, and `\textsc` really does set small letters as capitals). `\mathbb` is the exception and is now real - see the `msbm10` entry below. The `\text*` family treats its argument as text, so the spaces in it survive, as `\text` already did.
