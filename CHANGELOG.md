...
- The arrows `\longleftrightarrow`, `\longmapsto` and `\hookleftarrow`, each built by butting a shaft against an arrowhead the way the ones already here are, since no bundled font has them whole.
- The spaces `\enspace` (half a quad) and `\space`, and amsmath's `\mspace{<length>}`, which is `\hspace` in math units.
- `\mathord`, `\mathop`, `\mathbin`, `\mathrel`, `\mathopen`, `\mathclose`, `\mathpunct` and `\mathinner`: the argument keeps its shape and changes its kind, which is what decides the space around it and whether a following script becomes a limit.
- The escaped literals `\#`, `\$`, `\&` and `\_` (`\%` was already here). The first three are ordinary glyphs of the text font; the underscore is not - OT1 has none - so it is drawn as a short rule below the baseline, which is what LaTeX does with it.
...
- A `\\` at the end of a matrix opened an empty row. `a & b \\ c & d \\` is a normal way to write one out, and the separator there closes the last row rather than starting another; the blank row left behind was a line the grid grew to fit, and the delimiters grew again to cover that, which left the last row sitting near the middle of its brackets. `array` had always dropped it; the matrix environments now do too.
- A matrix set its rows solid, with only a little padding between them, where TeX struts every row of a table to a line. Rows now take at least a line between them - the strut being leading, it applies where there is a neighbour to keep away from, and not above the first row or below the last, where it would only inflate the box and the delimiters drawn around it.
- An operator with no limits at all came out at the size of the letters beside it. Choosing the display form of the glyph happens where the scripts are set, and a lone `\int` has none - so it took the short way out and was drawn small.
- An integral stacked its limits above and below in display style, as `\sum` does. TeX gives an integral `\nolimits` and `\sum` `\limits`, in every style, which is why `\int_0^\infty` reads at its side in every published paper. `\int`, `\oint` and the multiple integrals built from them now set theirs beside; `\limits` after one still stacks them.
- A matrix's columns were three times too close together. TeX separates them by `2\arraycolsep`, 10pt, and amsmath removes the outer half of that at each end with `\hskip -\arraycolsep` so the brackets sit against the contents; this had 3.5pt between columns and 1.75pt outside them. An `array` takes the same gap but keeps its outer halves, which is the space inside the brackets of `\left[\begin{array}{cc|c} … \right]`. An aligned block is not a table and keeps the close spacing it had.
- A script whose argument was a font command lost that command's own argument. `m_\mathrm{el}` read `\mathrm` alone as the whole subscript and left `{el}` outside it, so the formula was rejected for a `\mathrm` with nothing to set. TeX reads one token after `_` and the macro then takes its argument; a font command in a script now does the same.
- Quotation marks. `` ` `` and `\"` were unknown characters, so a formula that quoted anything - `` ``|K^0\rangle'' `` - was rejected whole. They are the quotes of the text font, at the codes OT1 gives them.
- A script written after an accent was hung off nothing. The parser attaches scripts to the atom it has just built, but the accent path never did, so `\dot{C}^\mu` fell through to the \"there is no base to hand\" case: the superscript was given an empty box as its nucleus and set at the height of that, which left it level with the letter while the accent stood above it. It now attaches to the accented atom and clears the accent - rule 18a, a nucleus that is not a single character being measured as the box it came out as. Every accented symbol carrying a script in a corpus of 238,000 published formulas was being drawn too low.
- A fraction carried no `\nulldelimiterspace`. TeX sets one with 1.2pt on each side, in place of the delimiters it has not got (rule 15e); without it a fraction butts straight against its neighbours, and `{\frac{dt}{2t}}(8\pi^2t)` ran the `2t` into the bracket. Braces make a fraction an ordinary atom and there is no glue between an ordinary and an opening, so that space was the only thing that would have separated them. `\binom`, and a `\genfrac` that was given delimiters, ask for none of it: their delimiters stand where it goes.
