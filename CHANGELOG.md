...
- The font-switching commands `\mathbf`, `\mathbb`, `\mathsf`, `\mathtt`, `\mathfrak`, `\mathscr`, `\textrm`, `\textbf`, `\textit`, `\textsf`, `\texttt`, and `\textsc`. The bundled Computer Modern set has no bold, sans-serif, monospace, blackboard-bold or Fraktur face, so all of those map to the nearest face that does exist and render as upright roman (`\mathscr` is genuinely the calligraphic alphabet, and `\textsc` really does set small letters as capitals). A formula using `\mathbb{R}` now reads correctly even though it is not set in the right face, instead of failing to parse. The `\text*` family treats its argument as text, so the spaces in it survive, as `\text` already did.
